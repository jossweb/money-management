...
def on_button_click(button_text):
    label.config(text="Le bouton '{}' a �t� cliqu� !".format(button_text))

def AppWindow(users):
    # ... Votre code pr�c�dent ...

    # Cr�er le widget Canvas pour le fond avec des coins arrondis
    ...
    # Cr�er les boutons dans le Frame
    for user in users:
        button = tk.Button(button_frame, text=user,
                           ...
                           padx=10,         # Espace horizontal (marge int�rieure) du bouton
                           pady=5           # Espace vertical (marge int�rieure) du bouton
                           )
    ...
    # Configurer le Canvas pour g�rer le d�filement
    ...
    # Placer le Canvas au centre de la win�tre principale (vertical et horizontal)
